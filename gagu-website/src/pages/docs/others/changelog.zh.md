...
- v0.0.60 (即将发布)
  - 本次更新 **必须** 执行 `gagu --reset` 重置全部历史数据，或手动删除 `WORKSPACE/.gagu/data` 目录
  - 新增：音乐播放器支持 .m4a
  - 新增：网页浏览器
  - 新增：引入 motion 动画
  - 新增：所有长列表使用虚拟滚动（移除 thumbScrollWatcher）
  - 修复：`sha256` 替代 `md5`
  - 修复：部分场景下文件名未截断
  - 修复：同时删除多个文件调用单个接口
  - 修复：关闭非顶部窗口时，再次打开没有显示在顶部

- v0.0.59 (2025-07-04)
  - 新增：文本编辑器变更状态显示到标题，不再通过 toast 提示
  - 新增：PDF 在线预览（Doc Reader）
  - 修复：检查更新报错
  - 修复：删除未上传头像的用户报错
  - 修复：文件管理器排序功能无法点击
  - 修复：重复右键菜单无法显示
  - 修复：移动文件时，跨分区则先复制再删除，同分区则使用重命名来加快速度
  - 修复：网站文档提示需要安装 Ghostscript 才能生成 PDF 缩略图

- v0.0.58 (2025-06-03)
  - 由于 `io` 域名续费价格持续上涨（本人无法忍受这种价格垄断上的肆意勒索），及其将被 IANA 除名的潜在可能，经评估后，现已启用新域名 `gagu.jsw.im`，另 `gagu.io` 将在不晚于 **2026.5.18** 后完全停用
  - 原 `WORKSPACE/.io.gagu` 将被创建为 `WORKSPACE/.gagu`（历史数据需要手动迁移）
  - 新增：登录凭证管理
  - 修复：图片浏览器全屏预览无法拖拽
  - 修复：Linux 下默认目录访问出错
